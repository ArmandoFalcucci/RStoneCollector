...
reports_ui <- function() {
  tabsetPanel(id = "report_tabs",
      ...
      fluidRow(
        column(4,
          box(width = 12, title = "Plot Controls", status = "primary", solidHeader = TRUE,
              ...
              checkboxInput("bb_stats", "Show statistical test", TRUE),
    ...
    tabPanel("PCA",
      br(),
      uiOutput("pca_filter_status"),
      fluidRow(
        column(3,
          box(width = 12, title = "PCA Controls", status = "primary", solidHeader = TRUE,
              uiOutput("pca_var_ui"),
              selectInput("pca_group", "Colour points by:", choices = c("(none)" = "")),
              actionButton("pca_capture", "Add to Report", icon = icon("plus"),
                           style = "width:100%;background:#1565c0;color:white;border:none;margin-top:6px;"),
              br(), br(),
              tags$strong("Export scores plot:"),
              fluidRow(
                column(6, downloadButton("pca_dl_png", "PNG 300dpi", icon = icon("file-image"),
                                          style = "width:100%;background:#1b5e20;color:white;border:none;font-size:12px;margin-top:4px;")),
                column(6, downloadButton("pca_dl_jpg", "JPG 300dpi", icon = icon("file-image"),
                                          style = "width:100%;background:#1b5e20;color:white;border:none;font-size:12px;margin-top:4px;"))
              ),
              fluidRow(
                column(6, downloadButton("pca_dl_csv", "Loadings CSV", icon = icon("file-csv"),
                                          style = "width:100%;background:#37474f;color:white;border:none;font-size:12px;margin-top:4px;")),
                column(6, downloadButton("pca_dl_r", "R Script", icon = icon("code"),
                                          style = "width:100%;background:#37474f;color:white;border:none;font-size:12px;margin-top:4px;"))
              ))),
        column(9,
          box(width = 12, title = "Scores (PC1 vs PC2)", status = "info",
              plotOutput("pca_scores", height = "380px")),
          box(width = 12, title = "Variance explained", status = "info",
              plotOutput("pca_scree", height = "200px")),
          box(width = 12, title = "Loadings", status = "info",
              DT::DTOutput("pca_loadings_tbl")))
      )
    ),
    tabPanel("Correspondence Analysis",
      br(),
      uiOutput("ca_filter_status"),
      fluidRow(
        column(3,
          box(width = 12, title = "CA Controls", status = "primary", solidHeader = TRUE,
              selectInput("ca_x", "Row variable (categorical):", choices = character(0)),
              selectInput("ca_y", "Column variable (categorical):", choices = character(0)),
              actionButton("ca_capture", "Add to Report", icon = icon("plus"),
                           style = "width:100%;background:#1565c0;color:white;border:none;margin-top:6px;"),
              br(), br(),
              tags$strong("Export CA biplot:"),
              fluidRow(
                column(6, downloadButton("ca_dl_png", "PNG 300dpi", icon = icon("file-image"),
                                          style = "width:100%;background:#1b5e20;color:white;border:none;font-size:12px;margin-top:4px;")),
                column(6, downloadButton("ca_dl_jpg", "JPG 300dpi", icon = icon("file-image"),
                                          style = "width:100%;background:#1b5e20;color:white;border:none;font-size:12px;margin-top:4px;"))
              ),
              downloadButton("ca_dl_r", "R Script", icon = icon("code"),
                              style = "width:100%;background:#37474f;color:white;border:none;font-size:12px;margin-top:4px;"))),
        column(9,
          box(width = 12, title = "CA biplot (Dim1 vs Dim2)", status = "info",
              plotOutput("ca_plot", height = "440px")),
          box(width = 12, title = "Variance explained", status = "info",
              plotOutput("ca_scree", height = "200px")))
      )
    ),
      ...
      fluidRow(
        column(8,
          box(width = 12, title = "Captured items", status = "primary", solidHeader = TRUE,
              p(tags$em(paste(
                "Items captured from Build Your Own, PCA, and CA tabs land here.",
                "Toggle the checkbox to include or exclude in the compiled report.",
                "Drag... no, sorry: use the up/down arrows to reorder."))),
        ...
        column(4,
          box(width = 12, title = "Compile report", status = "success", solidHeader = TRUE,
              textInput("builder_report_title", "Report title:",
                        value = "", placeholder = "e.g. Boomplaas - LSA assemblage"),
              textInput("builder_report_subtitle", "Subtitle (optional):",
                        value = "", placeholder = "e.g. Comparison of HP and SB units"),
